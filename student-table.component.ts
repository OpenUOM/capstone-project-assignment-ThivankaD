...
export class StudentTableComponent implements OnInit {
  ...
  search(query) {
    ...
    if (!query) {
      ...
      // Filter studentData based on query matching student names
      filteredData = this.studentData.filter((student) =>
        student[0].name.toLowerCase().includes(query.toLowerCase())
      );
